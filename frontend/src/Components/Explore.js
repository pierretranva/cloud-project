...
const Explore = () => {
	...
	useEffect(() => {
		axios
			.get("http://localhost:3000/db/profiles")
			.then((res) => {
	...
	const filterProfiles = () => {
		return profiles.filter(
			(profile) =>
				...
				Object.entries(tags).every(([tag, isSelected]) => (isSelected ? profile.tags.includes(tag) : true))
		);
	};
				...
				<div style={{ height: "80vh", flex: "30%", padding: "1em", borderRight: "1px solid gray" }}>
					...
					<FormGroup>
						{Object.entries(tags).map(([tag, isSelected]) => (
							<FormControlLabel
								key={tag}
								control={<Checkbox checked={isSelected} onChange={() => handleTagChange(tag)} color="primary" />}
								label={tag}
					...
					<Slider value={ageRange} onChange={handleSliderChange} min={0} max={100} valueLabelDisplay="auto" />
				</div>
